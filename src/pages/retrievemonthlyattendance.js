...
class Retrievemonthlyattendance extends React.Component {
    constructor() {
        super();
        this.state = {
            startdate: '',
    ...
    handleYear(yearvalue) {
        ...
        alert(yearvalue);
        db.collection("studentabsentreport").where("section", "==", this.state.section).where("year", "==", yearvalue).where("date", ">=", this.state.startdate).where("date", "<=", this.state.enddate).get().then((snapshot) => {
            const students = [];
...
export default Retrievemonthlyattendance;