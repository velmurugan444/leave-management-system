...
export const Viewteachers = () => {
    ...
    useEffect(() => {
        var data = database.ref().child("teacherdetails");
                ...
                tutorDetails != "" ? tutorDetails.map((data, index) => {
                    return <tr key={index}>
                        ...
                        <td>{data.mobile} &ensp;&ensp;&ensp;&ensp;<i className="fa fa-trash" aria-hidden="true" onClick={() => {
                            const reference = database.ref('teacherdetails').child(data.id);
                            reference.remove(); window.location.reload();
                        }} ></i></td>
                    </tr>;
                }) : <div className="container1"><div className="alert alert-danger" id="alert">No Teachers Added Yet</div></div>
            }
        </tbody></table>
    </>;
};
