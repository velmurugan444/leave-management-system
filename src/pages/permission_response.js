...
const Permissionresponsepage = () => {
    ...
    db.collection("confirmpermissions").where("rollno", "==", rollno).where("section", "==", section).where("year", "==", year).get().then(snapshot => {
        const students = [];
    ...
    return <div>
        ...
        <br /><br /><br /><br /><br /><br />
        {
            display ? students.map((student, index) => {
                ...
                return (
                    <Card className="response-card" style={display ? { display: "block" } : { display: "none" }} key={index}>
                        <Card.Body>
                            <br />
                            <div>
            ...
            }) : <div className="container"><div className="alert alert-danger" style={{ marginLeft: "0%" }}>No Confirmation Letter Yet</div></div>
        }
    </div>
